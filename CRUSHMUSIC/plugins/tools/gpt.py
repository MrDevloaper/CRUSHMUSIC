...
@app.on_message(filters.command(["rashant"], prefixes=["p", "P"]))
async def gpt_handler(client: Client, message: Message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"Hello {name}, I am Prashant. How can I help you today?")
            return
...
async def chat_crush(client: Client, message: Message):
    try:
        await client.send_chat_action(message.chat.id, ChatAction.RECORD_AUDIO)
        name = message.from_user.first_name

        if len(message.command) < 2:
            await message.reply_text(f"Hello {name}, I am Crush. How can I assist you today?")
            return
