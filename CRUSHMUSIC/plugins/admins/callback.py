...
from  CRUSHMUSIC import YouTube, app
from  CRUSHMUSIC.core.call import PRASHANT
from  CRUSHMUSIC.misc import SUDOERS, db
from  CRUSHMUSIC.utils.database import (
    get_active_chats,
    get_assistant,
    get_lang,
    get_upvote_count,
...
from  CRUSHMUSIC.utils.decorators.language import languageCB
from  CRUSHMUSIC.utils.formatters import seconds_to_min
from  CRUSHMUSIC.utils.inline import close_markup, stream_markup, stream_markup_timer
from  CRUSHMUSIC.utils.inline.play import panel_markup_1
from  CRUSHMUSIC.utils.stream.autoclear import auto_clean
from  CRUSHMUSIC.utils.thumbnails import get_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await PRASHANT.pause_stream(chat_id)
        ...
        await PRASHANT.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await PRASHANT.stop_stream(chat_id)
        ...
        await PRASHANT.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await PRASHANT.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await PRASHANT.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await PRASHANT.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await PRASHANT.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await PRASHANT.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await PRASHANT.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
