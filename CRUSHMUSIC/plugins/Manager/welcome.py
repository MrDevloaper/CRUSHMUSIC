...
def welcomepic(pic_path, user, chatname, user_id, uname):
    background = Image.open(" CRUSHMUSIC/assets/crush/CrushNwel.png")
    pfp = Image.open(pic_path).convert("RGBA")
    pfp = circle(pfp, size=(835, 839))
    draw = ImageDraw.Draw(background)
    font_large = ImageFont.truetype(' CRUSHMUSIC/assets/crush/ArialReg.ttf', size=65)
    ...
    if member.new_chat_member and member.new_chat_member.status == enums.ChatMemberStatus.MEMBER:
        try:
            ...
            if user.photo:
                ...
                pic_path = " CRUSHMUSIC/assets/upic.png"
...
""",
            ...
            if pic_path and os.path.exists(pic_path) and " CRUSHMUSIC/assets/upic.png" not in pic_path:
                os.remove(pic_path)
