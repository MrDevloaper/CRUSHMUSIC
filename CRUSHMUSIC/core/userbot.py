...
class Userbot:
    ...
    async def start(self):
        ...
        if self.one:
            ...
            try:
                await self.one.join_chat("Crush_Forever_Support")
                await self.one.join_chat("Crush_Forever_Support")
                await self.one.join_chat("Crush_Forever_Support")
                await self.one.join_chat("Crush_Forever_Support")
                await self.one.join_chat("Forever_Crush")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant 1 failed to join chats: {e}")
            assistants.append(1)
            try:
                await self.one.send_message(config.LOGGER_ID, "Crush's Assistant 1 Started")
            except Exception as e:
        ...
        if self.two:
            ...
            try:
                await self.two.join_chat("Crush_Forever_Support")
                await self.two.join_chat("Crush_Forever_Support")
                await self.two.join_chat("Crush_Forever_Support")
                await self.two.join_chat("Crush_Forever_Support")
                await self.two.join_chat("Forever_Crush")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant 2 failed to join chats: {e}")
            assistants.append(2)
            try:
                await self.two.send_message(config.LOGGER_ID, "Crush's Assistant 2 Started")
            except Exception as e:
                LOGGER(__name__).error(
        ...
        if self.three:
            ...
            try:
                await self.three.join_chat("Crush_Forever_Support")
                await self.three.join_chat("Forever_Crush")
                await self.three.join_chat("Crush_Forever_Support")
                await self.three.join_chat("Crush_Forever_Support")
                await self.three.join_chat("Crush_Forever_Support")
            except Exception as e:
            ...
            assistants.append(3)
            try:
                await self.three.send_message(config.LOGGER_ID, "Crush's Assistant 3 Started")
            except Exception as e:
                LOGGER(__name__).error(
        ...
        if self.four:
            ...
            try:
                await self.four.join_chat("Forever_Crush")
                await self.four.join_chat("Forever_Crush")
                await self.four.join_chat("Crush_Forever_Support")
                await self.four.join_chat("Crush_Forever_Support")
                await self.four.join_chat("Crush_Forever_Support")
            except Exception as e:
            ...
            assistants.append(4)
            try:
                await self.four.send_message(config.LOGGER_ID, "Crush's Assistant 4 Started")
            except Exception as e:
                LOGGER(__name__).error(
        ...
        if self.five:
            ...
            try:
                await self.five.join_chat("Forever_Crush")
                await self.five.join_chat("Forever_Crush")
                await self.five.join_chat("Crush_Forever_Support")
                await self.five.join_chat("Crush_Forever_Support")
                await self.five.join_chat("Crush_Forever_Support")
            except Exception as e:
            ...
            assistants.append(5)
            try:
                await self.five.send_message(config.LOGGER_ID, "Crush's Assistant 5 Started")
            except Exception as e:
                LOGGER(__name__).error(
    ...
    async def stop(self):
        LOGGER(__name__).info("Crush's assistants stopping...")
        if self.one:
            try:
                await self.one.stop()
            except Exception as e:
